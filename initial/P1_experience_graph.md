...
- `docs/context/eg/schema.json` (노드/엣지 타입 정의)
- `docs/context/eg/seed/*.json` (초기값 시드)
- `docs/context/eg/BOOTSTRAP.md` (주입 절차)
...
2. **EG 저장소 선택**: `docs/context/eg/BOOTSTRAP.md`의 두 경로 중 하나. 초기엔 경로 A(bastion_graph.db 확장, 거버넌스 네임스페이스). bastion.graph API 시그니처를 확인하고 로더를 실동작 코드로 작성.
...
4. **검증**: `docs/context/eg/validate.py`를 실행해 참조 무결성·커버리지·핵심순회를 확인. **오류 0이어야 함**(경고 bastion 등급-존 괴리는 정상).
