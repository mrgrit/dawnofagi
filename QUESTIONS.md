...
### 1. 인프라 프로비저닝 범위 — **결정: 사업이 등급을 선언한다**

이분법(컨테이너만 / 서버까지)이 아니었다. **사업마다 다르다:**
AOC 플랫폼은 서버가 붙어야 하고, AX 데모는 컨테이너면 되고, AX 프로덕트는 VM 이상이다.

그래서 `org/businesses/*.yaml` 에 `infra` 프로파일을 넣었다. 작업 지시가 `allowed`
중 하나를 고르고, **고른 등급이 결재 라인을 결정한다.**
...
| 등급 | 수단 | 이 호스트에서 | 자동화 | 결재 |
|---|---|---|---|---|
| `none` | 기존 환경에서 수행 | — | ✔ | 본부장 |
| `container` | 도커 + 존 네트워크(`el34-<zone>`) | ✔ **가능** | ✔ 자동 | 본부장 |
| `vm` | KVM | 하드웨어 ○ · **툴 없음 · 메모리 부족** | ✘ 절차 출력 | 대표이사 |
| `server` | 물리/클라우드 | ✘ | ✘ 절차 출력 | 대표이사 |

**용량이 실제 제약이다.** 측정치:

```
vCPU 4 · 메모리 5GB 여유/15GB · 디스크 316GB 여유
el34 컨테이너 42개가 이미 돌고 있다
/dev/kvm 있음 (vmx/svm 플래그 8) · virsh·qemu·multipass **전부 미설치**
```

→ **컨테이너 동시 3~4개가 현실적 상한이고, VM 은 1개도 빠듯하다.**
`server` 는 이 호스트로는 불가능하다. 즉 **AOC 플랫폼 사업의 기본 등급(server)은
지금 인프라로 충족할 수 없다** — 앞서 분산 논의(최소 4대·권장 6대)와 같은 결론이다.

**구현 방침**: `container` 만 실제로 자동화한다. `vm`·`server` 는 프로비저너가
**절차와 사양을 출력하고 작업 지시를 `준비대기` 로 멈춘다** — 사람이 조달한 뒤
등록하면 이어서 돈다. 과금이 발생하는 비가역 행동을 에이전트가 집행하지 않는다.
