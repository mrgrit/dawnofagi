...
| Q9-③ | 해결 | P7 | 정산 = 원가 집계만 (c). 단가 3종은 사람이 채운다 | 해제(§Q9) |
| Q10 | 해결 | P7 | 호스트 라우팅 · pipe 경유 (1번). 경로 개통은 사람 | 해제(§Q10) |
...
**구현 (2026-08-02).** `org/ratecard.yaml` 에 **원가만** 적는다 — 청구 단가는 여기
없다. 원가표에 청구 단가가 섞이면 원가가 그대로 견적이 되어 나간다(테스트가 막는다).

핵심은 **0 과 `미정` 을 구별한 것**이다:

| | 뜻 |
|---|---|
| `0` | 값이 0 이라고 **판단했다** (예: `infra_hour.none`) |
| `미정` | 아직 안 정했다. **사용량은 재되 금액은 안 매긴다** |

구별이 없으면 "로컬 모델이라 공짜"와 "아직 전기료를 안 따졌다"가 같은 0 으로 보이고
원가가 실제보다 싸게 잡힌다. 미정이 하나라도 있으면 합계는 **하한**으로 표시되고,
**경비로 기표하지 않는다** — 반쪽 금액을 장부에 올리면 그게 사실로 굳는다.

**아직 사람이 정해야 하는 것** (정해지면 `org/ratecard.yaml` 만 고치면 된다):

* 로컬 모델(ollama) 원가 — GPU 전력 + 감가상각
* 인프라 시간당 원가 — `container` / `vm` / `server`
* 고객 청구 단가 — 계약이 생긴 뒤


---

## Q10 — 컨테이너 존과 외부 호스트 존의 L3 경로가 다르다 (해결 2026-08-02 — 1번)
...
### 결정 — 1번 (호스트 라우팅 · pipe 경유)

**pipe 존이 PEP 라는 통제 평면의 전제가 유지되기 때문이다.** 존을 넘는 트래픽이
검사 지점을 지난다는 것은 이 회사의 통제 모델 전체가 기대는 가정이다. 2번(오버레이)은
그 가정을 L2 로 우회하고, 3번(존 분리)은 hand-off 를 파일 교환으로 제한한다.

**연 것과 열렸다고 적은 것을 따로 둔다:**

| | 누가 | 무엇 |
|---|---|---|
| 개통 | **사람** | `sudo bash infra/el34/route-external.sh --apply` (방화벽 변경 = 규칙 2) |
| 선언 | **사람** | `infra/pool.yaml` 의 `routing.enabled: true` · `zones: [dmz]` |
| 인정 | 기계 | `infrapool.transit_open()` — 선언된 존만 |

기본은 **닫힘**이고 전부 여는 기본값을 두지 않는다. 선언만 하고 실제로 안 열려 있으면
작업이 실패하지만, 그건 **시끄러운 실패**다 — 조용히 안 닿는 것보다 낫다.

스크립트는 열 규칙을 **보여주기만 하는 것이 기본**이고, 대역은 `pool.yaml` 에 등록된
장비 주소에서 읽는다(지어내지 않는다). NAT 는 걸지 않는다 — 출발지가 보여야 감사가 된다.

**실측 버그 하나.** `transit_open` 이 `load_pool(root)[0]`(= limits 딕셔너리)에서
`routing` 을 찾고 있었다. 최상위 키라 **항상 빈 값**이었고, 경로를 열어도 인정되지
않았다. `pool_doc()` 을 따로 두어 고쳤다. 그리고 `plan()` 도 존이 정확히 같은 장비만
고르고 있어서, 검사만 통과시키고 **실제로는 아무것도 못 잡았다** — 둘 다 테스트가 지킨다.
