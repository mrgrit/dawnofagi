...
"서버를 새로 구성하거나 간단한거면 컨테이너를 하나 올리든지" 라고 하셨는데,
**지금 이 호스트에서 확인된 건 도커뿐이다.**

```
도커        42개 컨테이너 가동 · 존 네트워크 4개(el34-ext/pipe/dmz/int) 있음
디스크      316GB 여유 / 423GB
메모리      5GB 여유 / 15GB     ← 컨테이너 몇 개면 찬다
클라우드    설정 없음
```

* **(a) 컨테이너만** — 이 호스트에 존 네트워크로 띄운다. 지금 바로 가능.
  메모리 5GB 라 동시 3~4개가 현실적 상한.
* **(b) 서버 신규까지** — 클라우드 계정·크레덴셜이 필요하다. 어느 클라우드인가.
* **(c) 컨테이너는 자동, 서버는 절차만 출력하고 사람이 실행** ← **권장**

(c) 를 권하는 이유: 서버 생성은 과금이 발생하는 비가역 행동이고, 지금 구조에서
에이전트가 클라우드 크레덴셜을 쥐는 건 `ctl.*` 급 권한이다.
