...
| Q10 | 대기 | P7 | 컨테이너 존 ↔ 외부 호스트 존 L3 경로 | 비차단 — 그 조합만 거부 |
...
**DoD-3 은 이 결정 없이 끝냈다 (2026-08-02).** 라우팅이 정해지기 전까지
**그 조합만 거부**한다 — 풀에 없는 존의 `vm`·`server` 를 요구하면 `PoolError` 이고
메시지가 이 항목을 가리킨다. 닿지도 않는 자원을 잡아 두면 작업이 **조용히** 실패하는데,
거부하면 시끄럽게 실패한다. 1번(호스트 라우팅)을 고르면 `CONTAINER_ZONES` 제약을
풀고 pipe 존 경유를 명시하면 된다.
