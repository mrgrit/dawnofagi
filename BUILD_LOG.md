...
## 2026-08-03 — 테스트가 운영자 로그인을 망가뜨리고 있었다

"admin/12341234 아니냐"는 물음에서 시작했다. **아니었다** — 어제 맞춰 뒀는데
`pytest` 를 돌릴 때마다 다시 바뀌고 있었다.

원인은 `test_web.py` 의 `_login()` 이다. **호출할 때마다 비밀번호를 임의로 새로
세팅한다.** 픽스처에 상수를 두면 그게 곧 커밋된 자격증명이 되므로(05_conventions #1)
그 판단 자체는 옳다. 문제는 그 쓰기가 **실 계정 저장소로 간다**는 것 —
한 번 돌리면 `admin` 포함 7개 계정이 아무도 모르는 `t-<토큰>` 이 됐다.

**테스트는 통과하는데 시스템은 못 쓰는 상태**가 가장 나쁘다. P8 §6 이 "남은
문제"로 적어 둔 그것이 실제로 사람을 막았다.

### 보호가 실행 방법을 타면 보호가 아니다

`conftest.py` 를 저장소 루트에 두는 것으로는 부족했다. 하위 패키지마다 자기
`pyproject.toml` 에 pytest 설정이 있어서 **`pytest apps/groupware` 로 돌리면
rootdir 이 그 디렉터리가 되고 루트 conftest 가 로드되지 않는다**(실측).
판단 수집 차단(`DAWN_JUDGMENT_COLLECT=0`)에도 같은 구멍이 있었다.

그래서 로직을 `dawn_core.testsupport` 로 빼고 **루트와 쓰는 패키지 양쪽**의
conftest 가 부르게 했다. 세션 전에 파일을 뜨고 후에 되돌린다 — 테스트 중에는
여전히 임의 비밀번호를 쓰므로 위 보안 판단은 유지되고, 끝나면 운영 상태로 돌아온다.

검증: 하위 패키지만 돌려도 · 전체를 돌려도 `12341234` 가 살아남는다.
`pytest` **453개 통과** (신규 2개) · `make lint`.

**남은 것**: 테스트가 실 트리에 쓰는 것 자체는 그대로다. 계정 저장소만 막았고
승인 큐·EG 는 여전히 오염된다. 복원 대상을 늘릴지, 테스트를 격리할지는 별건이다.

---

## 2026-08-04 — 작업 컨테이너를 worker 로 · 지시문을 읽고 편성한다

### ① 작업은 worker(.203) 에서 돈다

`container` 등급은 이 호스트(.231) 도커에 떴다 — "도커가 곧 풀" 이라는 설계였다.
그런데 .231 은 dawnofagi 자신(관제·포털·EG)이 도는 곳이라, 작업이 늘수록
관제가 먼저 느려진다. worker 는 4코어·15GB 에 kt88 컨테이너 4개뿐이다.

원격 도커는 **SSH 로만** 붙는다. TCP 소켓(2375/2376)은 기본이 무인증이라 여는
순간 그 호스트에 대한 무제한 원격 실행이 된다. `.231` 의 키는

    command="docker system dial-stdio",no-pty,no-port-forwarding,...

로 묶었다 — 셸을 열지 못한다. 실측으로 `ssh 203 "id; cat /etc/shadow"` 는
아무것도 출력하지 않고 `docker ps` 만 통한다. `.203` 의 `ccc` 는 이미 docker
그룹이라 sudo 도 필요 없다.

### ② 존은 못 주면 만들지 않는다

worker 는 물리 LAN 이고 존 네트워크(`el34-*`)는 .231 도커 브리지다. 안 닿는다
(Q10 미해결). 그래서 **존을 선언한 작업은 거부한다** — 붙여 놓고 "존에 있다"고
기록하면 게이트도 심각도도 거짓이 된다.

그런데 `provision.py` 가 빈 존을 본부 기본값으로 채우고 있었다. 존을 안 적은
작업까지 `dmz` 가 파생돼 전부 거부됐다. 접수자가 존을 안 적은 것은 "아무
존이나 좋다"가 아니라 "존 격리가 필요 없다"에 가깝다 — 없던 요구를 파생으로
만들 이유가 없다. **명시한 존은 그대로 두고 거부시킨다** (그건 진짜 요구다).

`Allocation` 에 `placed_on` 을 더했다. 없으면 화면이 "컨테이너 ready" 라고만
말하고 사람은 이 호스트에 있는 줄 안다 — 찾으러 갔는데 없다.

### ③ 지시문을 읽고 편성안을 만든다 — `dawn-biz hire`

편성은 `--role`·`--team`·`--tools` 를 손으로 다 넣어야 했고 한 번에 한 명이었다.
지시문을 읽지 않았다.

    작업 지시 → [모델] → 편성 초안(YAML) → 본부장이 고치고 승인 → crew.form()

**초안과 집행을 가른다.** 편성은 권한을 만드는 행위라, 모델이 지시문을 잘못
읽어 과한 역할을 제안할 수 있고 그건 사람이 파일로 보고 고칠 수 있어야 한다.

**게이트가 경계다, 모델의 의견이 아니다.** 제안한 도구는 팀 게이트와 교집합만
남기고, 뺏은 것은 `dropped_tools` 로 적는다 — 조용히 좁히면 모델이 무엇을
하려 했는지 사람이 못 본다. 그게 보여야 "이 작업엔 원래 더 센 권한이 필요한
것 아닌가"를 판단한다. 승인 시점에 **한 번 더** 자른다: 초안은 파일이라
편집되고, 파일을 믿으면 경계가 파일로 옮겨 간다.

모델은 **편성될 팀**의 것을 쓴다(본부가 아니라). 배정이 팀 단위인 이유도 있지만,
더 좋은 모델로 편성안만 그럴듯하게 뽑으면 실제로 도는 에이전트가 감당 못 하는
계획이 나온다.

팀장(`lead`)은 0명 또는 1명. **자기 게이트 안의 결정만** 스스로 내린다 —
비가역·L3·경계 밖은 팀장이 있어도 사람에게 올라간다. 팀장을 두는 것은 결재를
없애는 게 아니라 게이트 안쪽의 잔결정을 사람이 안 봐도 되게 하는 것이다.

### 실증 — 작업 지시 #65 (COSSAX 개발)

`lead-ax` 결재 완료 → worker 에 `wo65` 할당(존 브리지 없음 명시) → 지시문
(SPEC 6000자)을 sonnet 이 읽고 6명 편성안. 모델이 SPEC 2.5 를 근거로
`skill.preview`/`skill.run` 을 스스로 뺐다. 초안은 `var/biz/crew/wo65.yaml`
에 있고 **아직 승인하지 않았다** — 본부장의 몫이다.

### 검증

hire 테스트 12개(손으로 고친 초안도 게이트가 이긴다 · 팀장 1명 · 코드펜스 파싱 ·
승인자 없이 권한 없음) · 전체 432 통과 / 7 실패(HEAD 와 동일, 무관).

### 남은 것

관제 화면의 결정 표시와 팀장 auto 분기는 아직이다 — 이번 범위는 ①② 였다.

---

## 2026-08-05 — #65 착수: 권한은 줬는데 쓸 통로가 없다

에이전트 5명이 실제로 돌았다. 3명 품질 미통과, 2명 통과, 작업 지시는
`reviewing_output` 으로 멈췄다. **코드는 한 줄도 안 나왔다.**

### 검증자가 스스로 진단했다

`wo65-guardrail-verifier`(P3)는 자기 임무가 불가능함을 알아채고 그 이유를
증거로 냈다:

```
fs.write · dev.git · dev.tests 스팬 0건
호출된 도구는 eg.record 4 · eg.search 4 가 전부
워커 루프의 ①(조회)·④(기록)만 돌고 ③(구현)이 없다
```

판정: **검증 불가 (대상 산출물 없음).** 이 판정이 정확했고, 판정자(judge)도
이 산출물을 통과시켰다 — 없는 것을 있다고 하지 않고 근거를 붙였기 때문이다.

### 원인 — 설계다, 버그가 아니다

`Worker.run()` 은 4단계다:

    ① eg_search  →  ②③ extra_skills (호출자가 넘긴 것)  →  모델  →  ④ eg_record

②③ 은 **호출자가 명시한 스킬만** 돈다. 그런데 `cmd_start` 는

    w.run(f"{r['title']}\n\n{r['body']}", purpose="work")

로 부른다 — `extra_skills` 가 없다. 그래서 조회하고, 생각하고, 기록한다.
그게 전부다.

즉 **모델이 도구를 부를 통로가 아예 없다.** `agent.yaml`·`gate.yaml` 의
`fs.write`·`dev.git`·`dev.tests` 는 *허용 목록*이고, 그것을 집행하는 쪽이 없다.
편성이 만든 것은 권한이었지 능력이 아니었다.

claude-code provider 가 `--allowedTools ""` 인 것도 같은 방향이다. 그건
의도된 것이다 — `LLMClient.complete()` 는 completion API 고, 도구 집행은
dawn 의 스킬 계층이 하기로 돼 있다. 문제는 그 계층을 부르는 곳이 없다는 것이다.

### 그래서 산출물이 나쁘지 않았다

세 명이 낸 것은 **질문과 계획**이다. 그리고 SPEC §0 이 정확히 그것을 요구한다
("코드를 쓰기 전에 질문 정리 → 계획 승인"). 에이전트는 지시를 지켰다.

판정자는 "산출물이 없다"로 감점했다. 둘 다 맞다 — SPEC 은 지금 코드를 쓰지
말라 하고, 판정 기준은 구현을 요구한다. **기준이 서로 어긋나 있다.**

산출된 질문은 실제로 쓸 만하다. 예: "COSS 는 자사가 아니라 AX 컨설팅의
고객인데 산출물을 모노레포에 두면 테넌트 격리(규칙 4)와 걸린다", "행정용
`task` 는 런타임 `Task` 와 이름이 같고 의미가 다르다 — 섞으면 자사 관제가
남의 대학 업무를 자사 태스크로 집계한다".

### 고친 것 — EG 기록 이름

Task 노드 5개의 이름이 **전부 같았다.** `eg_record` 가 `task[:80]` 을 이름으로
쓰는데, 지시문이 길어 앞 80자가 머리말이었다:

    [wo65-*] COSSAX 개발\n\n# COSS 사업단 행정 에이전트 구축 지시문\n\n> Claude…

EG 는 다음 작업의 참조 근거인데 검색해서 다섯 개가 똑같이 나오면 고를 수 없다.
첫 줄만 쓰도록 고치고 기존 5건도 정정·재색인했다.

### 정하지 않은 것

**워커가 도구를 실제로 집행하게 할 것인가**는 손대지 않았다. 그건 모델에게
파일 쓰기·명령 실행을 주는 것이고 통제 평면의 성격이 바뀐다 — 사람이 정할
일이다. 선택지는 대략 셋: (a) `cmd_start` 가 위상별 `extra_skills` 를 넘긴다
(b) 워커에 도구 호출 루프를 넣는다 (c) 구현은 사람이 하고 에이전트는 계획·검증만
한다. 지금 시스템은 사실상 (c) 인데 그렇게 **선언된 적이 없다.**

### 부수

judge 표본 2 → 5 (P8 조건 4). 승인 큐 목적 태그 10건 백필.
전체 444 통과 / 7 실패(HEAD 와 동일, 무관).
