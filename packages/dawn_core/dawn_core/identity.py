...
그룹웨어만 쓰던 코드였다. 그런데 관제 콘솔(`dawn_aoc`)도 같은 로그인을 인정해야
하는데, 콘솔이 그룹웨어를 import 하면 **패키지 순환**이 생긴다(그룹웨어가 이미
`dawn_aoc.console` 을 쓴다). 계정과 능력은 어느 앱의 것도 아니므로 공용 계층으로
내렸다. `dawn_groupware.auth` 는 여기를 다시 내보내는 얇은 껍데기다.
